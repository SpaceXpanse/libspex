...
from SpaceXpanse Core.  (Since univalue, as used to validate moves in SpaceXpanse Core
...
class DupJsonKeysTest (MoverTest):
  ...
  def run (self):
    ...
    self.rpc.spacexpanse.name_register ("p/test", "{}")
    self.generate (1)

    # Try a duplicated game ID in the move JSON.  In this case, the notification
    # sent on ZMQ should just include the last value (that is what SpaceXpanse Core
    # officially does, enforced by a test).
    mv1 = json.dumps ({"d": "k", "n": 1})
    mv2 = json.dumps ({"d": "j", "n": 1})
    self.rpc.spacexpanse.name_update ("p/test", '{"g":{"mv":%s,"mv":%s}}' % (mv1, mv2))
    self.generate (1)
    self.expectGameState ({"players": {
      "test": {"x": 0, "y": -1},
    }})
    ...
    self.move ("a", "l", 1)
    mv = '{"d":"h","d":"l","n":1}'
    self.rpc.spacexpanse.name_update ("p/test", '{"g":{"mv":%s}}' % mv)
    self.move ("z", "h", 1)
    self.generate (1)
    self.expectGameState ({"players": {
      "test": {"x": 1, "y": -1},
