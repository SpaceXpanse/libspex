...
#ifndef SPACEXPANSEGAME_DEFAULTMAIN_HPP
#define SPACEXPANSEGAME_DEFAULTMAIN_HPP
...
struct GameDaemonConfiguration
{
  ...
  /**
   * The URL at which SpaceXpanse Core's JSON-RPC interface is available.  This
   * should already include the credentials, as in:
   *
   *  http://user:password@localhost:port
   */
  std::string SpaceXpanseRpcUrl;

  /**
   * The JSON-RPC protocol version that should be used when talking to SpaceXpanse.
   * For a real SpaceXpanse Core, this should be 1.  In other situations (e.g.
   * when using SpaceXpanse X), it could be set to 2.
   */
  int SpaceXpanseJsonRpcProtocol = 1;

  /**
   * The minimum required SpaceXpanse Core version.  By default, this is
   ...
   * the old game daemon needs to be updated to work with newer SpaceXpanse Core.
   */
  unsigned MaxSpaceXpanseVersion = 0;

  /**
   * If true, wait for SpaceXpanse Core's JSON-RPC interface to be available.
   * If false (the default), then we fail immediately if the interface
   * is not yet available on startup.
   */
  bool SpaceXpanseRpcWait = false;
...
/**
 * Runs a default "main" function for SpaceXpanse game daemons.  This default main
...
/**
 * Runs a default main function for SQLite-based SpaceXpanse game daemons.  The
...
#endif // SPACEXPANSEGAME_DEFAULTMAIN_HPP
