...
#ifndef SPACEXPANSEGAME_GAME_HPP
#define SPACEXPANSEGAME_GAME_HPP
...
/**
 * The main class implementing a game on the SpaceXpanse platform.  It handles the
 * ZMQ and RPC communication with the SpaceXpanse daemon as well as the RPC interface
 * of the game itself.
 *
...
private:
   ...
   *
   * DISCONNECTED:  The GSP is currently not actively connected to SpaceXpanse Core.
   * This state occurs when the RPC connection to SpaceXpanse Core throws, or if
  ...
  /** The JSON-RPC client connection to the SpaceXpanse daemon.  */
  std::unique_ptr<SpaceXpanseRpcClient> rpcClient;
...
public:
   ...
   * V1 is what needs to be used with a real SpaceXpanse Core instance, while
   * unit tests and other situations (e.g. SpaceXpanse X) need V2.
   */
  ...
  /**
   * Returns the version of the connected SpaceXpanse Core daemon in the form
   * AABBCCDD, where AA is the major version, BB the minor version, CC the
   * revision and DD the build.  For instance, 1020300 would be SpaceXpanse Core
   * 1.2.3.
   *
   * This method CHECK fails if the RPC connection has not yet been set up.
   */
  unsigned GetSpaceXpanseVersion () const;

  /**
   * Returns the chain (network) type as enum of the
   * connected SpaceXpanse daemon.  This can be used to set up the storage database
   ...
   * connected SpaceXpanse Core, then no pending state will be available.
   */
  ...
  /**
   * Detects the ZMQ endpoint(s) by calling getzmqnotifications on the SpaceXpanse
   ...
   * in the SpaceXpanse Core notifications, then this raises a JSON-RPC error.
   */
...
#endif // SPACEXPANSEGAME_GAME_HPP
