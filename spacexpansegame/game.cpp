 ...
 */
DEFINE_int32 (spacexpanse_waitforchange_timeout_ms, 5'000,
 ...
 * try to reconnect.
 */
DEFINE_int32 (spacexpanse_zmq_staleness_ms, 120'000,
              "ZeroMQ staleness before reconnection attempt");

/**
 * If set to non-zero, this indicates the interval (in milliseconds)
 * at which a running Game should probe its connection to SpaceXpanse Core.
 */
DEFINE_int32 (spacexpanse_connection_check_ms, 0,
...
public:
  ...
  explicit ConnectionCheckerThread (Game& g)
    : game(g), intv(FLAGS_spacexpanse_connection_check_ms), shouldStop(false)
  {
                        ...
                        const jsonrpc::clientVersion_t version)
{
  ...
  rpcClient = std::make_unique<SpaceXpanseRpcClient> (conn, version);
  ...
  CHECK (chain != Chain::UNKNOWN)
      << "Unexpected chain type returned by SpaceXpanse Core: " << chainStr;
...
unsigned
Game::GetSpaceXpanseVersion () const
{
  std::lock_guard<std::mutex> lock(mut);
...
/**
 * Hash-to-height translation function that calls SpaceXpanse Core by RPC.
 */
unsigned
GetHeightForBlockHash (SpaceXpanseRpcClient& rpc, const uint256& hash)
{
    ...
    const ExtractJsonFromStateWithLock& cb) const
{
  ...
  /* Getting the height for the hash value might throw, if we revert
     back to SpaceXpanse RPC and that is down.  We want to handle this case
     gracefully, so we can detect and recover from a temporarily
     down SpaceXpanse Core.  This code might be called in that case e.g.
  ...
  if (zmq.IsRunning ())
    {
      ...
      cvStateChanged.wait_for (lock,
          std::chrono::milliseconds (FLAGS_spacexpanse_waitforchange_timeout_ms));
...
Game::Start ()
{
  ...
  if (FLAGS_spacexpanse_connection_check_ms > 0)
                            ...
                            const uint256& currentHash)
{
  ...
  /* FIXME:  If we have a sync target block, we should pass it as explicit
     "to" block here.  For now, though, this is not supported by SpaceXpanse X,
...
Game::ProbeAndFixConnection ()
{
  VLOG (1) << "Probing game connection to SpaceXpanse...";

  if (state == State::DISCONNECTED)
    {
      LOG (INFO) << "Attempting to re-establish the SpaceXpanse connection...";
      try
        {
          CHECK (DetectZmqEndpoint ())
              << "ZMQ endpoints not configured in SpaceXpanse";
          ConnectToZmq ();
        }
      catch (const std::exception& exc)
        {
  ...
  const auto maxStaleness
      = std::chrono::milliseconds (FLAGS_spacexpanse_zmq_staleness_ms);
