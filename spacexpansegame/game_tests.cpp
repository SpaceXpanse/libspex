// Copyright (C) 2018-2023 The SpaceXpanse developers
...
#include "rpc-stubs/spacexpanserpcserverstub.h"
...
DECLARE_int32 (spacexpanse_zmq_staleness_ms);
...
/**
 * Mock RPC server that takes the place of the SpaceXpanse Core daemon in unit tests.
 * Some methods are mocked using GMock, while others (in particular,
 * getblockchaininfo) have an explicit fake implementation.
 */
class MockSpaceXpanseRpcServerWithState : public MockSpaceXpanseRpcServer
{
...
public:
  ...
  using MockSpaceXpanseRpcServer::MockSpaceXpanseRpcServer;
...
protected:
  ...
  HttpRpcServer<MockSpaceXpanseRpcServerWithState> mockSpaceXpanseServer;
  ...
  {
    g.ConnectRpcClient (mockSpaceXpanseServer.GetClientConnector (),
...
using SpaceXpanseVersionTests = GameTests;

TEST_F (SpaceXpanseVersionTests, Works)
{
  Json::Value networkInfo(Json::objectValue);
  networkInfo["version"] = 1020300;
  EXPECT_CALL (*mockSpaceXpanseServer, getnetworkinfo ())
      .WillOnce (Return (networkInfo));

  Game g(GAME_ID);
  ConnectToMockRpc (g);
  EXPECT_EQ (g.GetSpaceXpanseVersion (), 1020300);
}
...
{
  Game g(GAME_ID);
  mockSpaceXpanseServer->SetBestBlock (0, BlockHash (0));
  ConnectToMockRpc (g);
...
TEST_F (ChainDetectionTests, Reconnection)
{
  /* For the death test, we need to make sure that we only run the server
     ...
     in one of them.  */
  mockSpaceXpanseServer->StopListening ();

  Game g(GAME_ID);
  EXPECT_DEATH (
    {
      mockSpaceXpanseServer->StartListening ();
      mockSpaceXpanseServer->SetBestBlock (0, BlockHash (0));
      ConnectToMockRpc (g);
      ConnectToMockRpc (g);
    },
...
TEST_F (DetectZmqEndpointTests, BlocksWithoutPending)
{
  ...
  EXPECT_CALL (*mockSpaceXpanseServer, getzmqnotifications ())
...
TEST_F (TrackGameTests, CallsMade)
{
  {
    InSequence dummy;
    EXPECT_CALL (*mockSpaceXpanseServer, trackedgames ("add", GAME_ID));
    EXPECT_CALL (*mockSpaceXpanseServer, trackedgames ("remove", GAME_ID));
  }
...
protected:
  ...
  InitialStateTests ()
    : g(GAME_ID)
  {
    EXPECT_CALL (*mockSpaceXpanseServer, getblockhash (GAME_GENESIS_HEIGHT))
...
TEST_F (InitialStateTests, AfterGenesis)
{
  ...
  EXPECT_CALL (*mockSpaceXpanseServer, game_sendupdates (GAME_GENESIS_HASH, GAME_ID))
      .WillOnce (Return (upd));

  mockSpaceXpanseServer->SetBestBlock (20, BlockHash (20));
  ReinitialiseState (g);
  EXPECT_EQ (GetState (g), State::CATRODNG_UP);
...
TEST_F (InitialStateTests, WaitingForGenesis)
{
  ...
  mockSpaceXpanseServer->SetBestBlock (9, BlockHash (9));
  SetStartingBlock (8, BlockHash (8));
  AttachBlock (g, BlockHash (9), emptyMoves);
  EXPECT_EQ (GetState (g), State::PREGENESIS);

  mockSpaceXpanseServer->SetBestBlock (10, TestGame::GenesisBlockHash ());
...
TEST_F (InitialStateTests, MissedNotification)
{
  ...
  mockSpaceXpanseServer->SetBestBlock (20, TestGame::GenesisBlockHash ());
...
TEST_F (InitialStateTests, UnspecifiedGenesisHash)
{
  ...
  mockSpaceXpanseServer->SetBestBlock (10, BlockHash (10));
...
{
  mockSpaceXpanseServer->SetBestBlock (GAME_GENESIS_HEIGHT,
...
TEST_F (GetCurrentJsonStateTests, HeightResolvedViaRpc)
{
  ...
  EXPECT_CALL (*mockSpaceXpanseServer, getblockheader (GAME_GENESIS_HASH))
...
protected:
  ...
  {
    EXPECT_CALL (*mockSpaceXpanseServer, trackedgames (_, _)).Times (AnyNumber ());
    EXPECT_CALL (*mockSpaceXpanseServer, getrawmempool ())
...
TEST_F (SyncingTests, CatchingUpForward)
{
  ...
  mockSpaceXpanseServer->SetBestBlock (12, BlockHash (12));
  ReinitialiseState (g);
  EXPECT_EQ (GetState (g), State::CATRODNG_UP);
  ExpectGameState (TestGame::GenesisBlockHash (), "");
...
TEST_F (SyncingTests, CatchingUpMultistep)
{
  /* Tests the situation where a single game_sendupdates call is not enough
     to bring the game state fully up to date.  SpaceXpanse Core's
     -maxgameblockattaches limit is one reason why this may happen
     (https://github.com/spacexpanse/spacexpanse/pull/66).  */
  ...
  mockSpaceXpanseServer->SetBestBlock (13, BlockHash (13));
  ReinitialiseState (g);
  EXPECT_EQ (GetState (g), State::CATRODNG_UP);
  ExpectGameState (TestGame::GenesisBlockHash (), "");
...
{
  EXPECT_CALL (*mockSpaceXpanseServer, game_sendupdates (_, GAME_ID))
      .WillOnce (Return (SendupdatesResponse (BlockHash (12), "reqtoken")));
...
TEST_F (GameStorageRetryTests, AttachBlock)
{
  ...
  mockSpaceXpanseServer->SetBestBlock (11, BlockHash (11));
...
protected:
  ...
  {
    FLAGS_spacexpanse_zmq_staleness_ms = MAX_STALENESS.count ();
    ...
    const Json::Value notifications = ParseJson (R"(
      [
        {"type": "pubgameblocks", "address": "ipc:///tmp/spacexpansegame_game_tests"}
      ]
    )");
    EXPECT_CALL (*mockSpaceXpanseServer, getzmqnotifications ())
