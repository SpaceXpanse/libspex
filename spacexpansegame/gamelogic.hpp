...
#ifndef SPACEXPANSEGAME_GAMELOGIC_HPP
#define SPACEXPANSEGAME_GAMELOGIC_HPP
...
/**
 * The possible chains on which a game can be on the SpaceXpanse platform.
 */
enum class Chain
{
  UNKNOWN = 0,

  /* Chains based on SpaceXpanse Core */
  MAIN,
  TEST,
  REGTEST,
...
private:
  ...
  /**
   * SpaceXpanse Core RPC connection, if it has been initialised already from the
   * Game instance.
   */
  SpaceXpanseRpcClient* rpcClient = nullptr;
...
protected:
  ...
  /**
   * Returns the configured RPC connection to SpaceXpanse Core.  Must only be called
   * after InitialiseGameContext was invoked with a non-null RPC client.
   */
  SpaceXpanseRpcClient& GetSpaceXpanseRpc ();
...
public:
   ...
   * instance after the RPC connection to SpaceXpanse Core is up.
   *
   * The RPC client instance may be null, but then certain features
   * (depending on GetSpaceXpanseRpc) will be disabled.
   *
 ...
 * of an actual game so that it can be plugged into libspacexpansegame to form
 ...
 * with the state that libspacexpansegame keeps, games should leverage the transactions
 ...
 * and the libspacexpansegame-stored data, and allows atomic transactions spanning
 * both of them.
 */
...
#endif // SPACEXPANSEGAME_GAMELOGIC_HPP
