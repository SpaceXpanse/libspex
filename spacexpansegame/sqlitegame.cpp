...
protected:
  ...
  SetupSchema () override
  {
    ...
    auto& db = GetDatabase ();
    db.Execute (R"(
      CREATE TABLE IF NOT EXISTS `spacexpansegame_gamevars`
          (`onlyonerow` INTEGER PRIMARY KEY,
           `gamestate_initialised` INTEGER NOT NULL);
      INSERT OR IGNORE INTO `spacexpansegame_gamevars`
          (`onlyonerow`, `gamestate_initialised`) VALUES (1, 0);

      CREATE TABLE IF NOT EXISTS `spacexpansegame_autoids` (
    ...
    auto stmt = db.PrepareRo (R"(
      SELECT `name`
        FROM pragma_table_info ('spacexpansegame_gamevars')
    ...
    if (stmt.Step ())
      ...
      db.Execute (R"(
        ALTER TABLE `spacexpansegame_gamevars`
...
SQLiteGame::Storage::IsGameInitialised (const SQLiteDatabase& db)
{
  auto stmt = db.PrepareRo (R"(
    SELECT `gamestate_initialised`
      FROM `spacexpansegame_gamevars`
  )");

  CHECK (stmt.Step ())
      << "Failed to fetch result from spacexpansegame_gamevars";
...
SQLiteGame::Storage::InitialiseGame ()
{
  ...
  db.Prepare ("SAVEPOINT `spacexpansegame-stateinit`").Execute ();
  try
    {
      ActiveAutoIds ids(game);
      game.InitialiseState (db);
      db.Prepare (R"(
        UPDATE `spacexpansegame_gamevars`
          SET `gamestate_initialised` = 1
      )").Execute ();
      db.Prepare ("RELEASE `spacexpansegame-stateinit`").Execute ();
      LOG (INFO) << "Initialised the DB state successfully";
    }
  catch (...)
    {
      LOG (ERROR) << "Initialising state failed, rolling back the DB change";
      db.Prepare ("ROLLBACK TO `spacexpansegame-stateinit`").Execute ();
      throw;
    }
}
...
SQLiteGame::AutoId::AutoId (SQLiteGame& game, const std::string& key)
{
  auto stmt = game.database->GetDatabase ().Prepare (R"(
    SELECT `nextid` FROM `spacexpansegame_autoids` WHERE `key` = ?1
  )");
  stmt.Bind (1, key);
...
SQLiteGame::AutoId::Sync (SQLiteGame& game, const std::string& key)
{
  ...
  auto stmt = game.database->GetDatabase ().Prepare (R"(
    INSERT OR REPLACE INTO `spacexpansegame_autoids`
