...
#ifndef SPACEXPANSEGAME_SQLITEGAME_HPP
#define SPACEXPANSEGAME_SQLITEGAME_HPP
 ...
 * data in the database and libspacexpansegame's state, the underlying SQLiteStorage
 * used by SQLiteGame should be used as the main storage in Game (so that it
 * also holds undo data and the current game state).
 *
 * With this GameLogic implementation, the "game state" as seen by libspacexpansegame
 ...
 * (but not through the SQLiteStorage, as that is handled by libspacexpansegame).
 *
...
protected:
   ...
   *
   * Note that table names starting with "spacexpansegame_" are reserved for use by
   * libspacexpansegame itself and must not be used by game implementations.
   */
...
#endif // SPACEXPANSEGAME_SQLITEGAME_HPP
