...
DECLARE_int32 (spacexpanse_sqlite_wal_truncate_ms);
 ...
 * table in the database mapping the user's account name in SpaceXpanse to a string,
...
protected:
  ...
  InitialiseGame ()
  {
    ...
    /* We don't want to use a mock SpaceXpanse server, so reinitialising the state
...
TEST_F (UnblockedStateExtractionTests, UnblockedCallbackOnSnapshot)
{
  ...
  FLAGS_spacexpanse_sqlite_wal_truncate_ms = 0;
...
private:
  ...
  MockSpaceXpanseRpcServer mockSpaceXpanseServer;
  SpaceXpanseRpcClient rpcClient;
...
protected:
  ...
  SQLitePendingMoveTests ()
    : httpServer(MockSpaceXpanseRpcServer::HTTP_PORT),
      httpClient(MockSpaceXpanseRpcServer::HTTP_URL),
      mockSpaceXpanseServer(httpServer),
