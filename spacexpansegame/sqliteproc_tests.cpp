...
protected:
  ...
  Store (SQLiteDatabase& db) override
  {
    auto stmt = db.Prepare (R"(
      INSERT OR IGNORE INTO `spacexpansegame_procvalues`
...
public:
  ...
  SetupSchema (SQLiteDatabase& db) override
  {
    ...
    db.Execute (R"(
      CREATE TABLE IF NOT EXISTS `spacexpansegame_procvalues`
...
protected:
  ...
  {
    proc.Finish ();
    ExpectRecords (db, "spacexpansegame_procvalues", "value", expected);
  }
...
protected:
  ...
  {
    ExpectRecords (db, "spacexpansegame_statehashes", "hash", expected);
  }
