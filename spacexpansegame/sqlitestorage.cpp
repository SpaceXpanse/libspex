 ...
 */
DEFINE_int32 (spacexpanse_sqlite_wal_truncate_ms, 0,
...
SQLiteStorage::SetupSchema ()
{
  ...
  db->Execute (R"(
    CREATE TABLE IF NOT EXISTS `spacexpansegame_current`
        (`key` TEXT PRIMARY KEY,
         `value` BLOB NOT NULL);
    CREATE TABLE IF NOT EXISTS `spacexpansegame_undo`
                                    ...
                                    const GameStateData& data)
{
  ...
  db->Prepare ("SAVEPOINT `spacexpansegame-setcurrentstate`").Execute ();

  auto stmt = db->Prepare (R"(
    INSERT OR REPLACE INTO `spacexpansegame_current` (`key`, `value`)
      VALUES ('blockhash', ?1)
  )");
  stmt.Bind (1, hash);
  stmt.Execute ();
  ...
  db->Prepare ("RELEASE `spacexpansegame-setcurrentstate`").Execute ();
}
                            ...
                            const unsigned height, const UndoData& data)
{
  ...
  auto stmt = db->Prepare (R"(
    INSERT OR REPLACE INTO `spacexpansegame_undo` (`hash`, `data`, `height`)
      VALUES (?1, ?2, ?3)
  )");
...
SQLiteStorage::ReleaseUndoData (const uint256& hash)
{
  ...
  auto stmt = db->Prepare (R"(
    DELETE FROM `spacexpansegame_undo`
      WHERE `hash` = ?1
  )");
...
SQLiteStorage::BeginTransaction ()
{
  ...
  db->Prepare ("SAVEPOINT `spacexpansegame-sqlitegame`").Execute ();
}

void
SQLiteStorage::CommitTransaction ()
{
  db->Prepare ("RELEASE `spacexpansegame-sqlitegame`").Execute ();
  CHECK (startedTransaction);
  startedTransaction = false;

  /* Check if a periodic checkpointing of the WAL file is due.  */
  if (FLAGS_spacexpanse_sqlite_wal_truncate_ms > 0)
    {
      const auto intv
          = std::chrono::milliseconds (FLAGS_spacexpanse_sqlite_wal_truncate_ms);
...
{
  db->Prepare ("ROLLBACK TO `spacexpansegame-sqlitegame`").Execute ();
  CHECK (startedTransaction);
  startedTransaction = false;
}
