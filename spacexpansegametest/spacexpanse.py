...
"""
Code for running the SpaceXpanse Core daemon as component in an integration test.
"""
...
class Node:
  """
  An instance of the SpaceXpanse Core daemon that is running in regtest mode and
  used as component in an integration test of a SpaceXpanse game.
  """

  def __init__ (self, basedir, rpcPort, zmqPorts, binary):
    self.log = logging.getLogger ("spacexpansegametest.spacexpansenode")
    self.datadir = os.path.join (basedir, "spacexpansenode")
    self.binary = binary

    self.config = {
      "listen": False,
      "rpcuser": "spacexpansegametest",
      "rpcpassword": "spacexpansegametest",
      "rpcport": rpcPort,
      "fallbackfee": 0.001,
    }
    ...
    self.log.info ("Creating fresh data directory for SpaceXpanse node in %s"
                    % self.datadir)
    shutil.rmtree (self.datadir, ignore_errors=True)
    os.mkdir (self.datadir)
    with open (os.path.join (self.datadir, "spacexpanse.conf"), "wt") as f:
      f.write ("[regtest]\n")
  ...
  def start (self):
    if self.proc is not None:
      self.log.error ("SpaceXpanse process is already running, not starting again")
      return

    self.log.info ("Starting new SpaceXpanse process")
    args = [self.binary]
    ...
    if "" not in wallets:
      self.log.info ("Creating default wallet in SpaceXpanse Core...")
      rpc.createwallet (wallet_name="", descriptors=False)

    # We need to explicitly close the client connection, or else
    # SpaceXpanse Core will wait for it when shutting down.
    rpc ("close") ()
  ...
  def stop (self):
    if self.proc is None:
      self.log.error ("No SpaceXpanse process is running cannot stop it")
      return

    self.log.info ("Stopping SpaceXpanse process")
    self.rpc.stop ()
    ...
    self.log.info ("Waiting for SpaceXpanse process to stop...")
    self.proc.wait ()
    self.proc = None
  ...
  def run (self):
    """
    Runs the SpaceXpanse node with a context manager.
    """
...
class Environment:
  """
  A "base-chain environment" that consists just of a SpaceXpanse Core instance.

  This is an abstraction that can be used for testing in situations where
  a GSP is run not connected directly to SpaceXpanse Core, but for instance
  to a SpaceXpanse X instance and some other base chain.  In the present case, however,
  it is just a simple wrapper around a normal SpaceXpanse Core node.
  """
