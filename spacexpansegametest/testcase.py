...
"""
Basic framework for integration tests of SpaceXpanse games.
"""

from . import game
from . import premine
from . import spacexpanse
...
SPACEXPANSED_BINARY_DEFAULT = "/usr/local/bin/spacexpansed"
XCORE_BINARY_DEFAULT = "/usr/local/bin/spacexpansex-core"
XETH_BINARY_DEFAULT = "/usr/local/bin/spacexpansex-eth"
DEFAULT_DIR = "/tmp"
DIR_PREFIX = "spacexpansegametest_"
...
class SpaceXpanseGameTest (object):
  """
  Base class for integration test cases of SpaceXpanse games.  This manages the
  SpaceXpanse Core daemon, the game daemon, basic logging and the data directory.

  The actual test should override the "run" method with its test logic.  It
  can control the SpaceXpanse Core daemon through rpc.spacexpanse and the game daemon through
  rpc.game.
  """
  ...
  def __init__ (self, gameId, gameBinaryDefault):
    ...
    desc = "Runs an integration test for the SpaceXpanse game g/%s." % gameId
    parser = argparse.ArgumentParser (description=desc)
    parser.add_argument ("--spacexpansed_binary", default=SPACEXPANSED_BINARY_DEFAULT,
                         help="spacexpansed binary to use in the test")
    parser.add_argument ("--xcore_binary", default=XCORE_BINARY_DEFAULT,
                         help="spacexpansex-core binary to use")
    parser.add_argument ("--xeth_binary", default=XETH_BINARY_DEFAULT,
                         help="spacexpansex-eth binary to use")
  ...
  def main (self):
    ...
    logfile = os.path.join (self.basedir, "spacexpansegametest.log")
    ...
    self.log = logging.getLogger ("spacexpansegametest.testcase")
  ...
  def setup (self):
    ...
    base environment (e.g. SpaceXpanse Core RPC) but before the actual test
  ...
  def runBaseChainEnvironment (self):
    ...
    By default, this is just a spacexpanse.Environment instance, but tests
    can override this method to e.g. link to a custom chain using SpaceXpanse X.
    """
  ...
  def runDirectCoreEnvironment (self):
    """
    Runs a base-chain environment that just consists of a SpaceXpanse Core
    ...
    if self.zmqPending == "none":
      self.log.info ("Disabling ZMQ for pending moves in SpaceXpanse Core")
    ...
    env = spacexpanse.Environment (self.basedir, next (self.ports), zmqPorts,
                            self.args.spacexpansed_binary)
    with env.run ():
      self.spacexpansenode = env.node
      self.rpc.spacexpanse = env.node.rpc
      yield env

  @contextmanager
  def runSpaceXpanseXCoreEnvironment (self):
    """
    Runs a base-chain environment that uses SpaceXpanse X to link back to
    a real SpaceXpanse Core instance.
    """

    if self.zmqPending != "one socket":
      raise AssertionError ("SpaceXpanse-X-Core only supports one-socket pending")

    from spacexpansex import core
    env = core.Environment (self.basedir, self.ports,
                            self.args.spacexpansed_binary, self.args.xcore_binary)

    with env.run ():
      self.spacexpansenode = env.spacexpansenode
      self.rpc.spacexpanse = env.spacexpansenode.rpc
      yield env

  @contextmanager
  def runSpaceXpanseXEthEnvironment (self):
    """
    Runs a base-chain environment that uses SpaceXpanse X to link to an
    Ethereum-like test chain (based on Ganache).
    """

    if self.zmqPending != "one socket":
      raise AssertionError ("SpaceXpanse-X-Eth only supports one-socket pending")

    from spacexpansex import eth
  ...
  def syncGame (self):
    """
    Waits for the game daemon to sync up to the current SpaceXpanse Core
    blockchain state.
    """
  ...
  def getGameState (self):
    ...
    to sync up with SpaceXpanse's best block first.
    """
  ...
  def generate (self, n):
    """
    Generates n new blocks on the SpaceXpanse network.
    """
  ...
  def collectPremine (self, addr=None):
    ...
    premine.collect (self.rpc.spacexpanse, addr, logger=self.log)
