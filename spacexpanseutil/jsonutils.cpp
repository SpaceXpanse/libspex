...
/** Number of satoshis in one full ROD.  */
constexpr int64_t COIN = 100'000'000;

/**
 * Upper bound for the maximum possible amount of ROD we consider valid.
 * This is not the exact total supply, just something we use to protect against
 * potential overflows.
 */
constexpr int64_t MAX_ROD_AMOUNT = 80'000'000 * COIN;
...
ChiAmountFromJson (const Json::Value& val, int64_t& sat)
{
  ...
  if (dval < 0.0 || dval > MAX_ROD_AMOUNT)
    {
